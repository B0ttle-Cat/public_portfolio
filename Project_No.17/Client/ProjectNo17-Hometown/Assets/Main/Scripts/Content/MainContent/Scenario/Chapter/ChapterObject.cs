...
namespace TFContent
{
	public class ChapterObject : ObjectBehaviour//, IOdccUpdate
	{
		[ReadOnly]
		...
		public ObserverValue<ChapterState> chapterSaveState;
		public bool pause;
		...
		{
			chapterSaveState.Value = ChapterState.Playing;
			pause = false;

			await Awaitable.NextFrameAsync();

			chapterSaveState.Value = ChapterState.Finished;
		}
		...
		public void TimelineObjectBinding()
		{
			...
			foreach(var trackItem in trackBindingPath)
			{
				...
				GameObject bindingObject = HierarchyPathFind.FindObject(path);
