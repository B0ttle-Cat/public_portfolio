...
namespace TFContent
{
	...
	public abstract class CommandController<TListener, TCommand> : CommandController, IPerInputUpdate where TListener : CommandListener<TCommand>
	{
		...
		private bool blockingSoubleSetValue;

		public virtual TCommand Value {
			get => value;
			set {
				if(!isActiveAndEnabled || blockingSoubleSetValue) return;
				blockingSoubleSetValue = true;
		...
		protected void InitValue(TCommand value)
		{
			this.value = value;
			blockingSoubleSetValue = false;
		}
