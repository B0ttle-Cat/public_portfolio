...
namespace TFContent
{
	public class ProjectileHitEventReceiver : ComponentBehaviour, IProjectileLifecycle
	{
		...
		private void OnCollisionEnter(Collision collision)
		{
			...
			{
				projectileObject.OnHit(collision, true);
				return;
			}
			LayerMask hitEffectLayerMask = hitData.hitEffectLayer;
			if(hitEffectLayerMask.HasLayer(hitLayer))
			{
				if(collision.collider.TryGetComponent<ReactionTarget>(out var reactionTarget))
				{
					reactionTarget.ThisContainer.TryGetComponent<IFFProvider>(out var iffProvider);
					var unitRelationType = orderData.GetHitTargetRelation(iffProvider);
					if(hitData.hitUnitRelationType.HasFlag(unitRelationType))
					{
						projectileObject.OnHit(collision, reactionTarget);
					}
					else
					{
				...
				{
					projectileObject.OnHit(collision, false);
				}
			}
		}
		...
		void IProjectileLifecycle.Hit(Collision collision, ReactionTarget reactionTarget)
		{
			lifeData.HitCount--;
			if(lifeData.IsDead())
			{
		...
		void IProjectileLifecycle.Hit(Collision something, bool isBlocker)
		{
			if(isBlocker)
			{
				lifeData.HitCount = 0;
