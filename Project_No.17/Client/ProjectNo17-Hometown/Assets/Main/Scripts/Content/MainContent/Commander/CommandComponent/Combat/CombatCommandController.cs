...
namespace TFContent
{
	public class CombatCommandController : CommandController<CombatCommandListener, CombatCommandData>//, IOdccUpdate
	{
		...
		private void OnAction_performed(InputAction.CallbackContext obj)
		{
			...
			if(combatCommandData.selectCombatIndex != -1)
			{
		...
		{
			if(combatCommandData.selectCombatIndex == -1 || !combatCommandData.IsPreview) return;
			combatCommandData.MousePos = mousePos;
			Value = combatCommandData;
		}
	}
