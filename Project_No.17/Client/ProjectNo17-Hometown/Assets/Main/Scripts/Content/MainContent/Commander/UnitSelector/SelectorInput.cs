...
namespace TFContent
{
	public class SelectorInput : ComponentBehaviour, IPerInputUpdate, ISaveLoadTarget
	{
		...
		private void OnPointStay(Vector2 point)
		{
			...
			if(width < minDragDelta*0.5f && height < minDragDelta*0.5f)
			{
				selectorController.ProvisionalSelectPoint(startPoint, minDragDelta, includeFlag, excludeFlag);
			}
			else
			{
				selectorController.ProvisionalSelectRect(startPoint, endPoint, size, minDragDelta, width, height, includeFlag, excludeFlag);
			}
		}
		private void OnCancelSelect()
		{
			selectorController.OnClearProvisionalTarget();
		}
