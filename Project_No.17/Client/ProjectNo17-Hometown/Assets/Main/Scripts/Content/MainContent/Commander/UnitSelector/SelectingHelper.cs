...
namespace TFContent
{
	public class SelectingHelper : ComponentBehaviour, ISaveLoadData
	{
		...
		{
			data.SaveValue(nameof(ignoreSelectPoint), ignoreSelectPoint);
			data.SaveValue(nameof(ignoreSelectRect), ignoreSelectRect);
		}
		void ISaveLoadData.OnLoad(in SaveLoadData data)
		{
			data.LoadValue(nameof(ignoreSelectPoint), out ignoreSelectPoint, false);
			data.LoadValue(nameof(ignoreSelectRect), out ignoreSelectRect, false);
		}
	}
}