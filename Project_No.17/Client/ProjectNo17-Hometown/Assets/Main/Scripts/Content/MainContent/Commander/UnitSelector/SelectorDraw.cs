...
namespace TFContent
{
	public class SelectorDraw : ComponentBehaviour, ISaveLoadTarget
	{
		[SerializeField] private Canvas drawSelectorCanvas;
		[SerializeField, ReadOnly] private CanvasScaler drawSelectorScaler;
		...
		protected override void BaseStart()
		{
			...
			drawSelectorScaler = drawSelectorCanvas.GetComponent<CanvasScaler>();
		}

		internal void DrawPoint(Vector2 startPoint, float minDragSize, bool includeFlag, bool excludeFlag)
		{
			if(!IsCheckDraw()) return;
			drawSelectorScaler.referenceResolution = new Vector2(Screen.width, Screen.height);
			drawFlag = true;
		...
		internal void DrawRect(Vector2 startPoint, Vector2 endPoint, Vector2 size, float minDragDelta, float width, float height, bool includeFlag, bool excludeFlag)
		{
			...
			drawPoint.sizeDelta = Vector2.one * minDragDelta;
		...
		{
			if(drawSelectorCanvas == null) return false;
			if(drawSelectorScaler == null) return false;
