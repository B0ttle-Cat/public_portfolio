...
namespace TFContent
{
	public class RTSCameraInputData : DataObject
	{
		...
		public struct CameraOption
		{
			...
			public Vector2 closeZoomRotateXMinMax;
			...
			public bool IsCloseZoom(float distance, out float nearToFarLerp)
			{
				float minZoom = zoomMinMax.x;

				nearToFarLerp = 0f;
				if(closeZoom<0) nearToFarLerp = 1f;
				else nearToFarLerp = (distance - minZoom)/(closeZoom - minZoom);

				return closeZoom > distance;
			}
			public float ClampRotationX(float rotationX)
			{
				return Mathf.Clamp(rotationX, rotateXMinMax.x, rotateXMinMax.y);
			}
			public float ClampRotationX(float rotationX, float closeLerp)
			{
				Vector2 _rotateMinMax = Vector2.Lerp(closeZoomRotateXMinMax, rotateXMinMax, closeLerp);
				return Mathf.Clamp(rotationX, _rotateMinMax.x, _rotateMinMax.y);
			}
