...
namespace BC.AIProvider
{
	...
	public class ROCProvider : ComponentBehaviour//, IOdccUpdate
	{
		...
		public bool ComputeSelectActuatorModule(UnitPairsTarget[] allPairsValue, bool computeOnlyAIControl, out ActuatorModule resultActuator, out UnitPairsTarget resultPair)
		{
			...
			//bool thisIsAI = !iffProvider.IsUserControl;
			//IEnumerable<ActuatorModule> list = actuatorProvider.ActiveActuatorList.Where(i => !computeOnlyAIControl || thisIsAI || i.Basic.AI_AlwaysUseAutoWhenIsUser);
			//if(!EventManager.TryGet<ICombatCompute>(out var iCombatCompute)) return false;
			//
			//Dictionary<UnitRelationType, UnitPairsTarget[]> unitPairsTargetOfRelationType = new Dictionary<UnitRelationType, UnitPairsTarget[]>();
			//float highPoint = float.MinValue;
			//ActuatorModule highPointActuator = null;
			//UnitPairsTarget highPointPairs = default;
			...
			//{
			//	if(module == null) continue;
			//	var targetType = module.Target.targetType;
			//	if(!unitPairsTargetOfRelationType.TryGetValue(targetType, out var targetPairsValue))
			//	{
			//		targetPairsValue = allPairsValue.Where(i => targetType.HasFlag(i.relationType)).ToArray();
			//		unitPairsTargetOfRelationType.Add(targetType, targetPairsValue);
			//	}
			//	if(targetPairsValue.Length == 0) continue;
			//	module.ComputePriorityPoint(iCombatCompute, in targetPairsValue, out float[] points, out int highestIndex);
			//	if(highestIndex >= 0)
			//	{
			//		float point = points[highestIndex];
