...
namespace BC.AIProvider
{
	public partial class ActuatorPriorityOrder : ScriptableObject
	{
		...
		private ActuatorPriorityOrderBy[] priorityOrderByList = new ActuatorPriorityOrderBy[0];
		...
		public void PriorityPoint(ICombatCompute combatCompute, in UnitPairsTarget[] unitPairsTarget, Dictionary<int, float[]> priorityPoint, out float[] points, out int highestIndex)
		{
			...
			for(int i = 0 ; i < length ; i++)
			{
				...
				AbilityProvider order = combatCompute.FindAbilityProvider(unitPairs.thisIndex);
				AbilityProvider target = combatCompute.FindAbilityProvider(unitPairs.targetIndex);
