...
namespace TFSystem
{
	public class SceneController : ComponentBehaviour, ISceneController
	{
		...
		{
			Debug.Log($"Start ISceneController.ChangeSceneState : {CurrentState} => {nextState}");
			bool result = await Async();
			async Awaitable<bool> Async()
			{
				await DelayCommand();
			...
			Debug.Log($"Ended ISceneController.ChangeSceneState : {CurrentState} => {nextState}");
			return result;
		}
